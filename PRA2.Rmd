...
EJEMPLO DE COMO SE ELIMINARIAN LOS OUTLIERS DE UNA VARIABLE

outliers <- boxplot(wine$fixed.acidity, plot=FALSE)$out
wine <- wine[-which(wine$fixed.acidity %in% outliers),]


#Análisis de los datos.
##Selección de los grupos de datos que se quieren analizar/comparar (planificación de los análisis a aplicar).

**Agrupación por graduación alcohólica.**
```{r}
wine.bajaG <- wine[wine$alcohol < 9.5,]
wine.altaG <- wine[wine$alcohol > 11.1,]
```
**Agrupación por nivel de acidez volátil**
```{r}
wine.bajaAV <- wine[wine$volatile.acidity < 0.39,]
wine.altaAV <- wine[wine$volatile.acidity > 0.64,]
```
...
FALTA HOMOGENEIDAD DE LA VARIANZA
FALTA HOMOGENEIDAD DE LA VARIANZA
FALTA HOMOGENEIDAD DE LA VARIANZA
FALTA HOMOGENEIDAD DE LA VARIANZA AQUÍ HAY UN EJEMPLO PERO HABRÏA QUE PENSAR BIEN LOS GRUPOS ELEGIDOS EN 4.1 y usarlos tanto aquí como en el siguiente apartado

Homogeneidad de la varianza (https://rpubs.com/Joaquin_AR/218466)
```{r}
fligner.test(x = list(wine["pH"],wine["alcohol"]))
```
...
fligner.test(fixed.acidity ~ volatile.acidity, data = wine)
...
**Correlaciones**
```{r}
boxplot(wine$alcohol~wine$quality)
```
```{r}
boxplot(wine$volatile.acidity~wine$quality)
```
